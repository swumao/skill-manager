...
```bash
npm install -g @opencode/skill-manager
```

安装后会自动运行 `postinstall` 脚本，将内置 `skill-manager` Skill 复制到 `~/.agents/skills/`。然后执行一次初始化：

```bash
skill install            # 初始化全局仓库
```

### 方式二：从 GitHub 直接安装

无需发布到 npm registry，从 GitHub 安装：

```bash
npm install -g git+https://github.com/opencode-ai/skill-manager.git
```
...
脚本会依次：
1. 通过 `npm link` 注册 `skill` 全局命令
2. 运行 `skill init` 交互式初始化
3. 将内置 `skill-manager` Skill 复制到 `~/.agents/skills/`
