...
[![npm](https://img.shields.io/npm/v/@swumao/skill-manager)](https://www.npmjs.com/package/@swumao/skill-manager)
...
```bash
npx @swumao/skill-manager <command>
```

首次运行会自动下载，适合快速体验或偶尔使用。

### NPM 全局安装

```bash
npm install -g @swumao/skill-manager
skill install
```

全局安装后可直接使用 `skill` 命令，首次执行 `skill install` 完成全局仓库初始化。
